...
1. **Configurar identidade**<br>
   ...
   * **Implementações do Edge**: a identidade é tratada por meio da configuração do namespace de identidade da Adobe Experience Platform. Nenhuma configuração separada do Serviço de ID do visitante é necessária. Consulte [visão geral da implementação do Edge](/help/implementation/edge/overview.md) para obter detalhes.

   * **Implementações somente do Analytics**: o Serviço de ID de visitante da Adobe deve ser habilitado para identificar visitantes de forma consistente nas soluções da CX Enterprise. O Serviço de ID de visitante atribui uma ID exclusiva e persistente para cada visitante do site e permite que essa ID seja compartilhada em todas as soluções da CX Enterprise nas quais você se inscreve.

     Para obter mais informações, consulte a [documentação do Serviço de ID de visitante da Adobe](https://experienceleague.adobe.com/docs/id-service/using/home.html?lang=pt-BR).
