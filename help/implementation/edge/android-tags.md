---
title: Configurar o Android para mídia de transmissão com tags
description: Configure a coleção de mídia de transmissão para o Android usando a extensão de tag Adobe Streaming Media for Edge Network.
...
# Configurar o Android para mídia de transmissão com tags

É possível configurar a coleção de mídia de transmissão para o aplicativo Android por meio de uma propriedade móvel Tags, com as configurações de mídia gerenciadas na interface da Coleção de dados. Esta página aborda a configuração de Tags. Para configurar a SDK no código, consulte [Configurar o Android para mídia de streaming](android.md).
...
1. Publique suas alterações, adicione as dependências de `Core`, `Edge`, `EdgeIdentity` e `EdgeMedia` ao seu aplicativo e registre-as no Mobile Core.

## Rastrear eventos de mídia

Com a propriedade publicada e o rastreador criado, rastreie cada evento de mídia usando seu método de rastreador. Consulte a guia **Android** em cada página de [evento](/help/implementation/events/overview.md) e [variável](/help/implementation/variables/overview.md) para obter as chamadas exatas.
...
>* [Configurar o Android para mídia de streaming (no código)](android.md)
