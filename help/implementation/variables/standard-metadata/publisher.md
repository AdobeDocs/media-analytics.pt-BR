...
Definir `publisher` dentro de `xdm.mediaCollection.sessionDetails` ao chamar [`sendEvent`](https://experienceleague.adobe.com/pt-br/docs/experience-platform/collection/js/commands/sendevent/overview):
...
Chame o ponto de extremidade [sessionStart](https://developer.adobe.com/data-collection-apis/docs/endpoints/media/sessions/#sessionstart) com `publisher` dentro de `xdm.mediaCollection.sessionDetails`:
...
Use `ADBMobile.media.AudioMetadataKeys.PUBLISHER` para definir o publicador na propriedade `StandardMediaMetadata` do objeto de mídia antes de chamar `trackSessionStart`:
...
Use `MEDIA_AudioMetadataKeyPUBLISHER` para definir o publicador nos metadados padrão do objeto de mídia antes de chamar `mediaTrackSessionStart`:
