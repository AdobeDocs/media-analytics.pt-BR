...
| --- | --- | --- |
| Conteúdo | Expiração <br>padrão do eVar: Visita | Definir a própria eVar. |
| Tipo de conteúdo | Expiração <br>padrão do eVar: Visualização de página | Definir a própria eVar. |
| Tempo gasto no conteúdo | Tipo <br>de evento: Contador | Definir o próprio evento. |
| Inicialização de vídeo | Tipo <br>de evento: Contador | Definir o próprio evento. |
| Término de vídeo | Tipo <br>de evento: Contador | Definir o próprio evento. |
...
| name | `name`: (obrigatório) o nome ou ID do anúncio. | N/D | Não disponível. |
...
| parentPod | `parentPod`: a posição no conteúdo principal em que o anúncio foi reproduzido. | N/D | Não disponível. |
| parentPodPosition | `parentPodPosition`: a posição no pod para a reprodução do anúncio. | N/D | Não disponível. |
| CPM | `CPM`: o CPM ou CPM criptografado (com prefixo “~”) que se aplica a essa reprodução. | N/D | Não disponível. |
| Media.click | `s.Media.click(name, offset)` | `s.tl()` | Use uma chamada personalizada de análise de link para rastrear cliques. |
