...
O back-end de mídia soma a duração de cada intervalo de buffer (de `media.bufferStart` até a próxima alteração de estado). O valor é relatado na chamada de fechamento. O Analysis Workspace mostra o valor como `HH:MM:SS`; Feeds de dados, Data Warehouse e APIs de relatórios mostram o valor em segundos.
