...
O back-end de mídia define o sinalizador `isSet` em `mediaReporting.states[]` para a entrada `closedCaptioning` como `true` na primeira vez que um evento `media.statesUpdate` com `closedCaptioning` em `statesStart` é recebido. A métrica é relatada na chamada de fechamento.
...
| Customer Journey Analytics | [`mediaReporting.states[]`](https://experienceleague.adobe.com/en/docs/experience-platform/xdm/data-types/media-reporting-details) entrada onde `name = "closedCaptioning"`, campo `isSet` |
