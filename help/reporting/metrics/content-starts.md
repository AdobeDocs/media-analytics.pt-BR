...
O back-end de mídia define `mediaReporting.sessionDetails.isPlayed = true` na primeira vez que um evento `media.play` para o conteúdo principal é recebido. A métrica é acionada nesse evento de reprodução, mas relatada na chamada de fechamento. Para calcular a taxa de descarte antes da exibição, use `(Media starts − Content starts) / Media starts`.
...
| Customer Journey Analytics | [`mediaReporting.sessionDetails.isPlayed`](https://experienceleague.adobe.com/pt-br/docs/experience-platform/xdm/data-types/session-details-reporting) |
