...
O back-end de mídia define o sinalizador `isSet` em `mediaReporting.states[]` para a entrada `fullscreen` como `true` na primeira vez que um evento `media.statesUpdate` com `fullscreen` em `statesStart` é recebido. A métrica é relatada na chamada de fechamento.
...
| Customer Journey Analytics | [`mediaReporting.states[]`](https://experienceleague.adobe.com/en/docs/experience-platform/xdm/data-types/media-reporting-details) entrada onde `name = "fullscreen"`, campo `isSet` |
